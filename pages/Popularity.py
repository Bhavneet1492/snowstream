...
def render(schema):
    ...
    with col1:
        search_choice=st.text_input(f"Enter any keyword contained in the {schema}'s name",value="art",key="schemaKey")

    with col2:
        numberOfSamples=st.slider("Enter the number of samples to search from",min_value=0,value=5000,max_value=65000,step=1000,key="schemaKeySample")
...
tab1, tab2, tab3, tab4  = st.tabs(["Concepts","Author","Institution","Works"])
with tab1:
    st.subheader("Concept's name")
    render("concept")
